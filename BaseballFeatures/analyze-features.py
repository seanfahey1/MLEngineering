...
def main():
    ...
    if Path(Path(__file__).parent / "local_copy.p").is_file():
    ...
    df[convert_to_floats] = df[convert_to_floats].apply(pd.to_numeric)
    df[convert_to_string] = df[convert_to_string].apply(str)
